...
export class TaskController {
  ...
  findByProject(@Param('id') id: string) {
    return this.service.find({ project_id: Number(id) });
  }

  @Get('account/:id')
  findByAccount(@Param('id') id: string) {
    return this.service.findByAccountId(Number(id));
  }

  @Delete('assign/:id/:actor_id')
  deleteAssign(
    @Param('id') id: string,
    @Param('actor_id') actorId: string,
  ) {
    return this.service.deleteAssign(Number(id), Number(actorId));
  }

...
export class AccountService {
  ...
  async create(payload: any) {
    ...
    if (!payload.password && payload.Password) payload.password = payload.Password;
  ...
  async login(identifier: string, password: string) {
    if (!identifier || !password) {
      throw new BadRequestException('Email/name va mat khau la bat buoc');
    }
    ...
    if (!account.password_hash) {
      throw new BadRequestException('Tai khoan chua co mat khau hop le');
    }
  ...
  async changePassword(id: number, oldPassword: string, newPassword: string) {
    if (!oldPassword || !newPassword) {
      throw new BadRequestException('Mat khau cu va mat khau moi la bat buoc');
    }
